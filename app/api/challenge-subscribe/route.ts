...
const CHALLENGE_LIST_ID = 1133142;
...
async function addToList(contactId: number) {
  await fetch(`https://rest.smoove.io/v1/Contacts/${contactId}`, {
    method: "PUT",
    headers: HEADERS,
    body: escapeForSmoove({ lists_ToSubscribe: [CHALLENGE_LIST_ID] }),
  });
}
...
async function notifyManualAdd(name: string, email: string) {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: NOTIFY_EMAIL,
      subject: "נרשמה לאתגר קוד הקשר — יש להוסיף ידנית לסמוב",
      html: `<div dir="rtl" style="font-family:Arial,sans-serif;font-size:15px;line-height:1.6">
        <p>מישהי נרשמה לאתגר אבל לא נוספה אוטומטית לרשימה בסמוב.</p>
        <p><strong>שם:</strong> ${name}<br/>
        <strong>מייל:</strong> ${email}</p>
        <p>יש להוסיף ידנית לרשימת האתגר בסמוב.</p>
      </div>`,
    }),
  });
}
  ...
  try {
    const res = await fetch("https://rest.smoove.io/v1/Contacts", {
      ...
      body: escapeForSmoove({
        ...
        firstName,
        lastName,
        lists_ToSubscribe: [CHALLENGE_LIST_ID],
      }),
    });
    ...
    // Smoove returns 409 / plain text when the contact already exists globally
    ...
    console.error("Smoove challenge subscribe error:", err);
  }
